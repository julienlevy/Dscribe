    ...
    init(vibrancy optionalVibrancy: VibrancyType?) {
        ...
        let setupViews: Void = {
            ...
            if #available(iOSApplicationExtension 8.2, *) {
                self.label.font = UIFont.systemFont(ofSize: 25, weight: UIFontWeightLight)
            }
...
//            self.connector!.drawUnder = false
    ...
    // DSCRIBE
    func setLabelFrame() {
        self.label.frame = CGRect(x: self.labelInset, y: self.labelInset, width: self.bounds.width - self.labelInset * 2, height: self.bounds.height - self.labelInset * 2 - keyLabelOffsetForCharacter(self.text, font: self.label.font))
    }
}
